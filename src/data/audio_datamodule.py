...
class NpyDataset(torch.utils.data.Dataset):
    def __init__(self, inputs, labels=None, filter_unvoiced: bool = False) -> None:
        assert labels is None or len(inputs) == len(
            labels
        ), "Lengths of inputs and labels do not match"
...
class AudioDataModule(LightningDataModule):
    def __init__(
        ...
        # sanity checks
        assert (
            val_audio_files is None or val_annot_files is not None
        ), "Validation set (if it exists) must be annotated"
        assert (
            val_audio_files is None or fold is None
        ), "Specify `val_audio_files` OR cross-validation `fold`, not both"
        assert (
            annot_files is not None or fold is None
        ), "Cannot perform cross-validation without any annotations."
    ...
    def prepare_data(self) -> None:
        ...
        if self.val_audio_files is not None:
            self.val_dataset = self.load_data(
                self.val_audio_files, self.val_annot_files
            )
    ...
    def train_dataloader(self):
        return torch.utils.data.DataLoader(
            self.train_dataset, sampler=self.train_sampler, **self.dl_kwargs
        )

    def val_dataloader(self):
        return torch.utils.data.DataLoader(
            self.val_dataset, sampler=self.val_sampler, **self.dl_kwargs
        )
    ...
    def load_data(
        self, audio_files: Path, annot_files: Path | None = None
        ...
        if cache_cqt.exists():
            ...
            annotations = (
                np.loadtxt(cache_annot, dtype=np.float32)
                if cache_annot.exists()
                else None
            )
        else:
        ...
        return NpyDataset(
            inputs, labels=annotations, filter_unvoiced=self.filter_unvoiced
        )
    ...
    def precompute_hcqt(
        self, audio_path: Path, annot_path: Path | None = None
        ...
        return np.concatenate(cqt_list), np.concatenate(
            annot_list
        ) if annot_list is not None else None
    ...
    def hcqt(self, audio: torch.Tensor, sr: int):
        ...
        if sr != self.hcqt_sr:
            ...
            self.hcqt_kernels = HarmonicCQT(
                sr=sr, hop_length=hop_length, **self.hcqt_kwargs
            )

        return (
            self.hcqt_kernels(audio).squeeze(0).permute(2, 0, 1, 3)
